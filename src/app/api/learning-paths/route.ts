...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!templateId) {
      return NextResponse.json(createErrorResponse('缺少 templateId'), { status: 400 })
    }

    const tpl = getTemplate(templateId)
    if (!tpl) {
      return NextResponse.json(createErrorResponse('模板不存在'), { status: 404 })
    }
    ...
    if (tpl.prerequisite) {
      ...
      if (!prereq) {
        return NextResponse.json(createErrorResponse('前置路径尚未完成'), { status: 400 })
      }
    }
